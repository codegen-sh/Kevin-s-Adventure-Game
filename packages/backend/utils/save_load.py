...
def save_game_data(player, world, save_name):
    """Save game data with a custom save name for API use."""
    ensure_save_directory()
    ...
    try:
        ...
        raise Exception(f"Error saving game: {e}")

def load_game_data(save_name):
    """Load game data by save name for API use."""
    filename = f"{save_name}.json"
    filepath = os.path.join(SAVE_DIRECTORY, filename)
    ...
    try:
        ...
        raise Exception(f"Error loading game: {e}")
    except json.JSONDecodeError:
        raise Exception(f"Save file {filename} is corrupted")
