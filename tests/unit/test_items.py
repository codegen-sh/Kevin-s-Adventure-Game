...
class TestUseItem:
    ...
    def test_use_map(self, mock_get_available, mock_print, player_with_items, sample_world):
        ...
        mock_print.assert_any_call("You consult the map. It shows the following locations you can go to:")
    ...
    def test_use_bread(self, mock_remove, mock_heal, mock_print, player_with_items, sample_world):
        """Test using bread item."""
        result = use_item(player_with_items, "bread", sample_world)
        
        assert result is True
        mock_print.assert_called_with("You eat the bread. It's delicious and restores some health.")
    ...
    def test_use_stick_in_forest(self, mock_add_item, mock_print, player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_any_call("You wave the stick around. It makes a satisfying swoosh sound.")
        mock_print.assert_any_call("A nearby bird is startled and drops a shiny object!")
        mock_add_item.assert_called_once_with(player_with_items, "gold_coin")
    
    @patch('builtins.print')
    def test_use_stick_outside_forest(self, mock_print, player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_called_once_with("You wave the stick around. It makes a satisfying swoosh sound.")
    ...
    def test_use_berries_heal(self, mock_random_event, mock_remove, mock_damage, 
                            mock_heal, mock_print, player_with_items, sample_world):
    ...
    @patch('game.player.remove_item_from_inventory')
    @patch('utils.random_events.generate_random_event')
    def test_use_berries_poison(self, mock_random_event, mock_remove, mock_damage, 
                              mock_heal, mock_print, player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_any_call("You eat the berries. They're sweet and juicy.")
        mock_print.assert_any_call("Uh oh, those weren't safe to eat. You lose some health.")
    ...
    def test_use_torch_in_cave(self, mock_print, player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_called_with("You light the torch, illuminating the dark cave around you.")
    ...
    def test_use_torch_outside_cave(self, mock_print, player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_called_with("You light the torch. It provides warmth and light.")
    
    @patch('builtins.print')
    @patch('builtins.input')
    @patch('game.player.remove_item_from_inventory')
    def test_use_gemstone_sell_in_village(self, mock_remove, mock_input, mock_print, 
                                        player_with_items, sample_world):
        ...
        assert result is True
        mock_print.assert_any_call("You examine the gemstone closely. It glimmers with an otherworldly light.")
        mock_print.assert_any_call("A merchant notices your gemstone and offers to buy it for 50 gold!")
    ...
    @patch('builtins.print')
    @patch('builtins.input')
    def test_use_gemstone_keep_in_village(self, mock_input, mock_print, 
                                        player_with_items, sample_world):
    ...
    def test_use_unknown_item(self, mock_print, player_with_items, sample_world):
        ...
        assert result is False
        mock_print.assert_called_once_with("You're not sure how to use the unknown_item.")
...
class TestAddItemToWorld:
    ...
    def test_add_new_item_to_location(self, mock_print, sample_world):
        ...
        assert len(sample_world["locations"]["Village"]["items"]) == len(initial_items) + 1
...
class TestTransferItem:
    ...
    @patch('game.player.remove_item_from_inventory')
    def test_transfer_from_inventory_to_world_success(self, mock_remove, 
                                                    player_with_items, sample_world):
        ...
        result = transfer_item(player_with_items, sample_world, "map", from_inventory_to_world=True)
    ...
    @patch('game.player.remove_item_from_inventory')
    def test_transfer_from_inventory_to_world_fail(self, mock_remove, 
                                                 player_with_items, sample_world):
        ...
        result = transfer_item(player_with_items, sample_world, "nonexistent", from_inventory_to_world=True)
        
        assert result is False
        assert sample_world["locations"]["Village"]["items"] == initial_items
    
    @patch('game.player.add_item_to_inventory')
    def test_transfer_from_world_to_inventory_success(self, mock_add, 
                                                    sample_player, sample_world):
        """Test successfully transferring item from world to inventory."""
        sample_world["current_location"] = "Village"
        
        result = transfer_item(sample_player, sample_world, "map", from_inventory_to_world=False)
    ...
    def test_transfer_from_world_to_inventory_fail(self, sample_player, sample_world):
        ...
        result = transfer_item(sample_player, sample_world, "nonexistent", from_inventory_to_world=False)
...
class TestItemIntegrity:
    ...
    def test_all_items_have_descriptions(self):
        ...
        known_items = [
            ...
            "rope", "pickaxe", "mushrooms", "mountain_herbs", "ancient_coin",
            "hermit's_blessing", "gold_coin", "silver_necklace", "ancient_artifact",
            "magic_ring", "mysterious_potion", "sword"
        ]
        
        for item in known_items:
            description = get_item_description(item)
            assert description != "A mysterious item.", f"Item {item} has no specific description"
