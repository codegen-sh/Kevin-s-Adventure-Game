...
class TestInteractWithLocation:
    ...
    def test_interact_with_village(self, mock_visit_village, sample_world, sample_player):
    ...
    def test_interact_with_mountain(self, mock_climb_mountain, sample_world, sample_player):
    ...
    def test_interact_with_unknown_location(self, mock_print, sample_world, sample_player):
        """Test interacting with an unknown location."""
        sample_world["current_location"] = "UnknownPlace"
        
        interact_with_location(sample_world, sample_player)
        
        mock_print.assert_called_once_with("There's nothing special to interact with here.")
...
class TestWorldIntegrity:
    ...
    def test_location_descriptions_are_not_empty(self, sample_world):
        ...
        for location_name, location_data in sample_world["locations"].items():
            assert len(location_data["description"].strip()) > 0, \
    ...
    def test_current_location_exists_in_locations(self, sample_world):
        ...
        assert current_location in sample_world["locations"], \
