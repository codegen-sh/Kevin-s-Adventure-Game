...
def mock_random():
    ...
    with patch("random.random") as mock_random, patch(
        "random.choice"
    ) as mock_choice, patch("random.randint") as mock_randint:
        ...
        yield {"random": mock_random, "choice": mock_choice, "randint": mock_randint}
...
def mock_file_operations():
    ...
    with patch("builtins.open", create=True) as mock_open, patch(
        "os.path.exists"
    ) as mock_exists, patch("os.listdir") as mock_listdir, patch(
        "json.dump"
...
def assert_player_health_in_range(player, min_health=0, max_health=100):
    ...
    assert (
        min_health <= player["health"] <= max_health
...
def assert_valid_location(world, location):
    ...
    assert location in world["locations"], f"Location '{location}' not found in world"
